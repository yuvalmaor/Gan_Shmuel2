...
def monitor(service):
    ...
    try:
        urllib.request.urlopen(
            f"http://ec2-13-200-131-223.ap-south-1.compute.amazonaws.com:{SERVICES_PORT[service]}/health", timeout=10)
    except:
...
def build_docker_image(service: str) -> str:
    ...
    gunicorn_logger.info(
        f"Building Docker image '{image_tag}' from '{service}'")
...
def deploy_docker_compose(service: str) -> None:
    ...
    subprocess.run(["docker-compose", "-f",
                   f"{GIT_PATH}/{service}/docker-compose.yml", "up", "-d"])
...
def testing():
    ...
    result = subprocess.run(["docker-compose", "-f", f"{GIT_PATH}/billing/test-docker-compose.yml",
                            "up", "-d"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if result.returncode != 0:

        gunicorn_logger.error(
            f"Errors or failures occurred during the compose tests.")
    ...
    result = subprocess.run(["docker-compose", "-f", f"{GIT_PATH}/weight/test-docker-compose.yml",
                            "up", "-d"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if result.returncode != 0:
    ...
    # run pytest
    command = ["pytest", f"{GIT_PATH}/billing/tests",
               f"{GIT_PATH}/weight/tests"]
